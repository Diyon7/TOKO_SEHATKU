...
function showDataTeamCheck(hanyaKetua = false){
  ...
  if(hanyaKetua){
      innerAnggota += '<tr>'+
                      '<th>Anggota</th>'+
                      '<td><font class="deep-orange-text text-lighten-2" style="font-size: 12px">*tidak ada anggota yang ditambahkan</font></td>'+
                      '</tr>';
  }
  else{
    for(var i = 1; i<=n; i++){
...
function changeContent(value, animation = "slideInDown"){
  ...
  var tahap1 = data.valid_tahap1;
  var tahap2 = data.valid_tahap2;
  value = "#"+value;
  // Remove class
  ...
  if((pembayaran > 1 && value == "#data-pembayaran") || anggota > 1 && value == "#data-anggota" || tahap1 > 1 && value == "#penyisihan-tahap1" || tahap2 > 1 && value == "#penyisihan-tahap2"){
    value += "-lengkap";
  }
...
function innerInputAnggota(){
  ...
  n -= 1;
  var innerInput = "";
  var innerButton = '<a class="waves-effect btn deep-orange animated zoomIn" onclick="validateDataAnggota()">&nbsp;NEXT&nbsp;</a>';
  var innerInfo = '<div class="card secondary deep-orange white-text animated fadeInDown">'+
                  '<div class="card-content">'+
                  '<p>'+
  ...
  for(var i = 1; i<=n; i++){
    innerInput += '<p class="animated fadeInDown title grey-text text-darken-2">Anggota '+i+'</p>'+
                  '<div class="divider deep-orange animated zoomIn"></div>'+
                  '<br>'+
                  ...
                  '<img src="img/drag/drag-photo-fwc.png" id="image-preview-anggota'+i+'" class="file-field input-field" accept="image/*">'+
                  '<br><br>'+
